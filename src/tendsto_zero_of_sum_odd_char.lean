...
/-!
# Theorems regarding sums of odd characters
...
lemma helper_5_odd {k : ℕ} (hk : 0 < k) : (d * p^m)^k = (d * p^m) * (d * p^m)^(k - 1) :=
begin
...
variables [fact (0 < d)] {R : Type*} [normed_comm_ring R]
...
lemma norm_sum_le_smul_odd {k : ℕ} [normed_algebra ℚ_[p] R] [norm_one_class R] (hk : 1 < k) {x : ℕ}
  (na : ∀ a b : R, ∥(a + b)∥ ≤ max (∥a∥) (∥b∥)) :
  ∥∑ (y : ℕ) in finset.range (d * p ^ x + 1), (asso_dirichlet_character
  (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ((-1) * ↑y) *
  ∑ (z : ℕ) in finset.range k, ↑(d * p ^ x) ^ z * ((-1) * ↑y) ^ (k - (z + 1)) *
  ↑(k.choose (z + 1))∥ ≤ (dirichlet_character.bound
  (χ.mul (teichmuller_character_mod_p_inv p R ^ k)) * k) :=
begin
  have : ∀ y ∈ finset.range (d * p ^ x + 1), ∥(asso_dirichlet_character
    (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ((-1) * ↑y) *
    ∑ (z : ℕ) in finset.range k, ↑(d * p ^ x) ^ z * ((-1) * ↑y)^(k - (z + 1)) *
    ↑(k.choose (z + 1)) ∥ ≤ (dirichlet_character.bound (χ.mul
    (teichmuller_character_mod_p_inv p R ^ k))) * k,
  { intros l hl,
    ...
    have : ∀ a ∈ finset.range k, ∥(-1 : R) ^ (k - (a + 1)) * (↑(d * p ^ x) ^ a *
      (↑l ^ (k - (a + 1)) * ↑(k.choose (a + 1))))∥ ≤ 1,
    { intros a ha,
    ...
    rw [finset.sum_const, finset.card_range, nat.smul_one_eq_coe], },
...
lemma sum_eq_neg_sum_add_dvd (hχ : χ.is_odd) [algebra ℚ_[p] R] [nontrivial R]
  ...
  (asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ↑y * ↑y ^ k +
  ...
  ∑ (x_1 : ℕ) in finset.range k, ↑(d * p ^ x) ^ x_1 * ((-1) * ↑y) ^ (k - (x_1 + 1)) *
  ↑(k.choose (x_1 + 1))) :=
begin
  ...
  { rw [←int.cast_one, ←int.cast_neg, mul_eval_neg_one, asso_odd_dirichlet_character_eval_neg_one
      _ hχ,  ← units.coe_neg_one, ← units.coe_neg_one, asso_dirichlet_character_eq_char _ (-1),
      change_level_pow_eval_neg_one k hp, units.coe_pow, units.coe_neg_one], 
      ring_nf, 
      rw [← pow_mul', even.neg_one_pow _],
      refine even_two_mul _,
...
lemma helper_11_odd [nontrivial R] [no_zero_divisors R] [normed_algebra ℚ_[p] R]  [norm_one_class R]
 (na : ∀ a b : R, ∥(a + b)∥ ≤ max (∥a∥) (∥b∥))
 [fact (0 < m)] {k : ℕ} (hk : 1 < k) (hχ : χ.is_odd) (hp : 2 < p) {x : ℕ} (hx : m ≤ x) :
 ∃ y, (2 : R) * ∑ i in finset.range (d * p^x), ((asso_dirichlet_character (χ.mul
  (teichmuller_character_mod_p_inv p R ^ k))) i * i^k) = ↑(d * p^x) * y ∧ ∥y∥ ≤ ((χ.mul
  (teichmuller_character_mod_p_inv p R ^ k)).bound * (↑k)) + ∥(((d * p ^ x : ℕ) : R) ^ (k - 1)) *
...
begin
  have f1 : ∑ (i : ℕ) in finset.range (d * p ^ x), (asso_dirichlet_character (χ.mul
    ...
    (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ↑i * ↑i ^ k
   - ((asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ↑(d * p^x) *
   ↑(d * p^x) ^ k),
  { rw [finset.sum_range_succ, add_sub_cancel], },
  rw [f1, mul_sub, mul_comm _ (↑(d * p ^ x) ^ k), ←mul_assoc _ (↑(d * p ^ x) ^ k) _,
    mul_comm _ (↑(d * p ^ x) ^ k), mul_assoc _ (2 : R) _, ←nat.cast_pow],
  clear f1,
  conv { congr, funext, rw [sub_eq_iff_eq_add, @helper_5_odd p _ d _ k (lt_trans zero_lt_one hk),
  ...
  refine ⟨∑ (y : ℕ) in finset.range (d * p ^ x + 1),
    (asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) (-1) *
    ((asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ↑y *
    ∑ (x_1 : ℕ) in finset.range k,
    ↑(d * p ^ x) ^ x_1 * ((-1) * ↑y) ^ (k - (x_1 + 1)) * ↑(k.choose (x_1 + 1))) -
    ↑((d * p ^ x) ^ (k - 1)) * ((1 + 1) * (asso_dirichlet_character (χ.mul
  ...
  { apply le_trans (norm_sub_le _ _) _,
    ...
    --apply le_trans (add_le_add _ (le_refl _)) _,
    refine le_trans (add_le_add (norm_sum_le_smul_odd hk na) (le_refl _)) _,
    rw ← mul_assoc,
...
lemma helper_7_odd [normed_algebra ℚ_[p] R] [norm_one_class R] (k : ℕ) (y : ℕ) :
  ∥((d * p ^ y : ℕ) : R) ^ (k - 1) * (1 + 1)∥ ≤ 1 :=
begin
  norm_cast,
...
lemma helper_9_odd [normed_algebra ℚ_[p] R] [norm_one_class R] {k : ℕ} (hk : 1 < k) (hp : 2 < p)
  (y : ℕ) : ∥(2⁻¹ : ℚ_[p])∥ * (↑k + ∥((d * p ^ y : ℕ) : R) ^ (k - 1) * (1 + 1)∥) ≤ k + 1 :=
begin
  rw ← one_mul ((k : ℝ) + 1),
  ...
  { refine add_le_add le_rfl (helper_7_odd k _), },
  { refine add_nonneg (le_trans zero_le_one _) (norm_nonneg _),
    norm_cast,
    refine (le_of_lt hk), },
  { linarith, },
end
...
variable (χ)
lemma helper_10_odd [normed_algebra ℚ_[p] R] [norm_one_class R] {k : ℕ} {ε : ℝ} (hk : 1 < k)
  (hp : 2 < p) (hε : 0 < ε) : ∃ x : ℕ,
  ∥(2⁻¹ : ℚ_[p])∥ * (↑k + ∥((d * p ^ x : ℕ) : R)^(k - 1) * (1 + 1)∥) *
...
begin
  ...
  have pos' : 0 < (↑k + 1) * (χ.mul (teichmuller_character_mod_p_inv p R ^ k)).bound,
  { apply mul_pos _ (dirichlet_character.bound_pos _), norm_cast, linarith, },
    --apply mul_pos (nat.cast_pos.2 (lt_trans zero_lt_one hk)) (dirichlet_character.bound_pos _), },
  have pos : 0 < ε / ((↑k + 1) * (χ.mul (teichmuller_character_mod_p_inv p R ^ k)).bound) := div_pos hε pos',
  refine ⟨classical.some (exists_pow_lt_of_lt_one pos one_div_lt_one), _⟩,
  refine lt_of_le_of_lt (mul_le_mul
    (helper_9_odd hk hp _) le_rfl (mul_nonneg (norm_nonneg _)
...
lemma helper_8_odd {k : ℕ} (hk : 1 < k) (n : ℕ) :
  0 ≤ (k : ℝ) + ∥(n : R) ^ (k - 1) * (1 + 1)∥ :=
begin
...
lemma sum_odd_character_tendsto_zero [nontrivial R] [no_zero_divisors R] [normed_algebra ℚ_[p] R]
  [norm_one_class R] [fact (0 < m)] {k : ℕ} (hk : 1 < k) (hχ : χ.is_odd) (hp : 2 < p)
  (na : ∀ a b : R, ∥(a + b)∥ ≤ max (∥a∥) (∥b∥)) :
  filter.tendsto (λ n : nat, ∑ i in finset.range (d * p^n), ((asso_dirichlet_character
  (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) i * i^k) ) filter.at_top (nhds 0) :=
begin
  -- better way to do this with filters
  refine metric.tendsto_at_top.2 (λ ε hε, _),
  obtain ⟨z, hz⟩ := helper_10_odd χ hk hp hε,
  refine ⟨max z m, λ x hx, _⟩,
  cases helper_11_odd na hk hχ hp (max_le_iff.1 hx).2,
  ...
  apply lt_of_le_of_lt (mul_le_mul le_rfl h.2 (norm_nonneg _) (mul_nonneg (norm_nonneg _)
    (norm_nonneg _))) _,
  rw [mul_comm _ (k : ℝ), ←add_mul, mul_mul_mul_comm],
  apply lt_of_le_of_lt (mul_le_mul (mul_le_mul le_rfl (add_le_add le_rfl _) (helper_8_odd hk _)
    ...
    (le_of_lt (dirichlet_character.bound_pos _))) (mul_nonneg (norm_nonneg _) (helper_8_odd hk _))) hz,
...
lemma sum_eq_neg_sum_add_dvd_of_units_odd (hχ : χ.is_odd) [normed_algebra ℚ_[p] R] [nontrivial R]
  [no_zero_divisors R] [fact (0 < m)] (hp : 2 < p) (k : ℕ) (hk : 1 ≤ k) {x : ℕ} (hx : m ≤ x) :
  ∑ (i : (zmod (d * p ^ x))ˣ), (asso_dirichlet_character (χ.mul
  (teichmuller_character_mod_p_inv p R ^ k))) ↑i * ↑i ^ k =
  -1 * ∑ (y : (zmod (d * p ^ x))ˣ),
  (asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ↑y *
  ↑y ^ k + ↑(d * p ^ x) * ∑ (y : (zmod (d * p ^ x))ˣ),
  (asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) (-1) *
  ((asso_dirichlet_character (χ.mul (teichmuller_character_mod_p_inv p R ^ k))) ↑y *
  ∑ (x_1 : ℕ) in finset.range k, ↑(d * p ^ x) ^ x_1 * ((-1) * ↑y) ^ (k - (x_1 + 1)) *
  ↑(k.choose (x_1 + 1))) :=
begin
  ...
  { --apply dvd_trans _ (mul_dvd_mul_left d (pow_dvd_pow p hk)),
    apply dvd_trans (conductor.dvd_lev _) _, --(dvd_trans (conductor_dvd _) _),
    rw helper_4, },
  refine eq.trans (helper_16_odd k hx) _,
  ...
  rw [←int.cast_one, ←int.cast_neg, mul_eval_neg_one _ _, asso_odd_dirichlet_character_eval_neg_one _ hχ], 
  conv_lhs { congr, congr, skip, rw [← units.coe_neg_one, asso_dirichlet_character_eq_char _ (-1)], },
  convert_to (-1 : R) * (-1 : R)^k * (-1)^k = -1,
  { apply congr_arg2 _ _ rfl,
    rw change_level_pow_eval_neg_one k hp,
    simp only [units.coe_neg_one, units.coe_pow],
    any_goals { apply_instance, }, },
  { ring_nf, rw ←pow_mul', rw even.neg_one_pow _, refine even_two_mul _, },
  { apply_instance, },
  { apply fact_iff.2 (nat.prime.pos _), refine fact_iff.1 _, apply_instance, },
end

--`helper_W_4` replaced with `helper_17`
lemma helper_17_odd [normed_algebra ℚ_[p] R] [norm_one_class R] {k : ℕ} {x : ℕ} (y : (zmod (d * p^x))ˣ) : 
  ∥(d : R) * ∑ (z : ℕ) in finset.range k,
  (((p ^ x : ℕ) : R) * ↑d) ^ z * ((-1) * ↑((y : zmod (d * p^x)).val)) ^ (k - (z + 1)) *
  ↑(k.choose (z + 1))∥ ≤ 1 :=
begin
...
lemma sum_odd_character_tendsto_zero_of_units [nontrivial R] [no_zero_divisors R] [normed_algebra ℚ_[p] R]  [norm_one_class R]
 --(n : ℕ) --[fact (0 < n)]
  ...
  i * i^k) ) (@filter.at_top ℕ _) (nhds 0) :=
begin
  ...
  { apply (tendsto_congr' _).2,
    ...
    { conv { congr, funext, rw finset.mul_sum, },
      ...
      { exact set.range_nonempty (λ (i : (zmod (d * p ^ x))ˣ), ∥↑(d * p ^ x) *
          ...
          finset.range k, ↑(d * p ^ x) ^ x_1 * ((-1) * ↑i) ^ (k - (x_1 + 1)) *
          ↑(k.choose (x_1 + 1))))∥), },
      { intros b hb,
        cases hb with y hy,
        ...
        refine le_trans (mul_le_mul (mul_le_mul le_rfl (helper_17_odd y) (norm_nonneg _)
    ...
    { simp_rw two_mul,
      ...
      conv { congr, --skip, funext,
        conv { congr, skip, rw sum_eq_neg_sum_add_dvd_of_units_odd hχ hp _ (le_of_lt hk) hx, }, },
