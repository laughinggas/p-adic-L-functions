...
lemma eq_reduction_change_level {m n : ℕ} (h : m ∣ n) (χ : dirichlet_character S m) :
  change_level h χ = change_level (dvd_trans (conductor.dvd_lev _) h) χ.reduction :=
begin
  rw reduction,
  conv_lhs { rw factors_through.spec χ (mem_conductor_set_factors_through _ (conductor.mem_conductor_set _)), },
  rw ← change_level.trans,
end
...
lemma mul_conductor_eq_mul_conductor (n : ℕ) :
  ...
  (χ * change_level (dvd_mul_of_dvd_right (dvd_pow_self p (nat.ne_zero_of_lt' 0)) d) (teichmuller_character_mod_p_inv p R ^ n)).conductor :=
begin
  rw (is_primitive_def _).1 (is_primitive.mul _ _),
  have : lcm (d * p^m) p = d * p^m := helper_4 m,
  have h2 : d * p^m ∣ lcm (d * p^m) p, { rw this, }, 
  rw change_level.trans (teichmuller_character_mod_p_inv p R ^ n) (dvd_mul_of_dvd_right (dvd_pow_self p (nat.ne_zero_of_lt' 0)) d) h2,
  rw ←monoid_hom.map_mul, congr',
  apply change_level_heq _ this.symm,
end

lemma exists_mul_of_dvd' (n : ℕ) (hd : d.coprime p) :
  ∃ (x y : ℕ), x ∣ d ∧ y ∣ p^m ∧ (χ.mul (teichmuller_character_mod_p_inv p R ^ n)).conductor = x * y :=
begin
  ...
  rw change_level.trans _ (dvd_pow_self p hm) (dvd_mul_left (p^m) d), rw ← monoid_hom.map_mul,
  ...
  rw change_level.trans χ₁ (dvd_lcm_left d (p^m)) _,
  rw change_level.trans _ (dvd_lcm_right d (p^m)) _,
  any_goals { rw this, },
...
begin
  ...
  simp_rw ← and_assoc,
  refine ⟨χ₁, χ₂, _, h⟩,
  rw eq_reduction_change_level at h,
  rw eq_reduction_change_level S _ χ₂ at h,
  have p1 : χ₁.conductor * χ₂.conductor ∣ m * n := mul_dvd_mul (conductor.dvd_lev _) (conductor.dvd_lev _),
  rw change_level.trans χ₁.reduction (dvd_mul_right _ _) p1 at h,
  rw change_level.trans _ (dvd_mul_left _ _) p1 at h,
...
begin
  ...
  refine ⟨χ₁, χ₂, _, h⟩,
  cases hχ with k hk,
  set η' := cast (dirichlet_character_eq_of_eq S hk) χ.reduction with hη',
  have mpos : 0 < m,
  ...
  { have p1 : change_level (mul_dvd_mul_left m dv) η' = χ,
    { rw hη', conv_rhs { rw ← change_level.self χ, rw eq_reduction_change_level, },
      rw change_level_cast _ _ hk, },
    rw h at p1, rw h'.2.2 at p1, rw monoid_hom.map_mul at p1,
    rw ← change_level.trans at p1, rw ← change_level.trans at p1,
    rw change_level.trans χ₂' dv (dvd_mul_left n m) at p1,
...
lemma dvd_mul_of_dvd_conductor (n : ℕ) (hd : d.coprime p) (hχ : d ∣ χ.conductor) :
  d ∣ (χ.mul (teichmuller_character_mod_p_inv p R ^ n)).conductor :=
begin
  have hm : m ≠ 0,
  { apply ne_zero_of_lt (fact.out _), exact 0, apply_instance, apply_instance, },
  ...
  { obtain ⟨x, y, hx, hy, h'⟩ := exists_mul_of_dvd' p d R m χ n hd,
    ...
    rw change_level.trans _ (dvd_pow_self p hm) (dvd_mul_left (p^m) d) at h',
    ...
    { rw ← h', rw (is_primitive_def _).1 (is_primitive.mul _ _),
      ...
      rw change_level.trans χ₁ (dvd_mul_right _ _) _,
      rw change_level.trans ψ (dvd_mul_left _ _) _,
      any_goals { rw this },
      rw ← monoid_hom.map_mul,
      congr',
      apply change_level_heq _ this.symm, },
    ...
    { have : change_level (dvd_trans (conductor.dvd_lev _) (nat.lcm_dvd_mul _ _)) (χ₁.mul ψ) =
      ...
      { rw dirichlet_character.mul, rw ← eq_reduction_change_level, rw monoid_hom.map_mul,
        rw ← change_level.trans, rw ← change_level.trans, },
      rw [← this, hη', change_level_cast _ _ h''], },
    rw hη at this, rw monoid_hom.map_mul at this,
    rw ← change_level.trans at this, rw ← change_level.trans at this,
    rw change_level.trans _ hx (dvd_mul_right d (p^m)) at this,
    rw change_level.trans _ hy (dvd_mul_left (p^m) d) at this,
...
begin
  ...
  --make separate lemma
  rw change_level.trans ψ (dvd_lcm_left n m) _,
  rw change_level.trans χ (dvd_lcm_right n m) _,
  any_goals { rw this },
  rw ←monoid_hom.map_mul,
  congr' 3,
  apply change_level_heq _ this.symm,
